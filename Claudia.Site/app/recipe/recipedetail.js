(function () {
    ...
    angular.module('app').controller(controllerId,
        ['$window', '$scope', '$route', '$location', '$routeParams', 'common', 'config', 'datacontext', 'models', 'bootstrap.dialog', 'helper', '$http', recipedetail]);

    function recipedetail($window, $scope, $route, $location, $routeParams, common, config, datacontext, model, bsDialog, helper, $http) {
        var vm = this;
        ...
        function getRequestedLink() {
            ...
            if (val === 'new') {
                vm.isNew = true;
                setUpImageTool();
                return $scope.model = datacontext.recipe.createEntity();
            }

            common.showBusy(true);
            return $scope.model = datacontext.recipe._getById(entityName, val)
                .then(foundEntity);
