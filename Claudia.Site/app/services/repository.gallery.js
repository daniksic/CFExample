(function () {
    ...
    function RepositoryGallery(models, AbstractRepository) {
        ...
        var showDeleted = Predicate.create("isDeleted", "==", false),
            objectCategory = Predicate.create("categoryId", "==", 1), //change to type
            activePredicate = Predicate.create("isDeleted", "==", false).and("categoryId", "==", 1);
        ...
        function getAll(forceRemote, page, size, nameFilter) {
            ...
            function querySucceeded(data) {
                self.isLoaded = true;
                self.log('Retrieved [Gallery] list from remote data source', data.results.length, false);
                return self.$q.when(getByPage());
            }
