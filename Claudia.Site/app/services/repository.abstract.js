(function () {
    ...
    function AbstractRepository(common, config) {
        ...
        function _getById(entityName, id, forceRemote) {
            ...
            if (!forceRemote) {
                ...
                if (entity && !entity.isPartial) {
                    self.log('Retrieved [' + entityName + '] id:' + entity.id + ' from cache.', null, false);
            ...
            function querySucceeded(data) {
                ...
                self.log('Retrieved [' + entityName + '] id ' + entity.id
                    + ' from remote data source', entity, false);
                //self.zStorage.save();
                return entity;
            }
        }
        ...
        function _getLocalEntityCount(resource, predicate) {
            var entities;
            if (predicate) {
                entities = EntityQuery.from(resource)
                    .where(_predicates.isNotNullo)
                    .where(predicate)
                    .using(this.manager)
                    .toType(resource)
                    .executeLocally();
