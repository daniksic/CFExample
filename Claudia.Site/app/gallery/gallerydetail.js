(function () {
    ...
    var controllerId = 'gallerydetail';

    angular.module('app').controller(controllerId,
        ['$window', '$scope', '$route', '$location', '$routeParams', 'common', 'config', 'datacontext', 'models', 'bootstrap.dialog', 'helper', '$http', gallerydetail]);

    function gallerydetail($window, $scope, $route, $location, $routeParams, common, config, datacontext, model, bsDialog, helper, $http) {
        var vm = this;
        ...
        vm.imgtool = { imagetool: undefined, init: false };
        ...
        function canSave() {
            return vm.hasChanges && !vm.isSaving && (vm.imgtool.init ? $scope.files.length > 0 : true);
        }
        ...
        function getRequestedLink() {
            ...
            if (val === 'new') {
                vm.isNew = true;
                setUpImageTool();
                return $scope.model = datacontext.gallery.createEntity();
            }

            common.showBusy(true);
            return $scope.model = datacontext.gallery._getById(entityName, val)
                .then(foundEntity);
             ...
             return $http({
                ...
                transformRequest: function (data) {
                    ...
                    for (var i = 0; i < data.files.length; i++) {
                        ...
                        formData.append("fileAttr" + i, angular.toJson({ crop: data.model.crop, size: data.model.size, imageCategory: "gallery" }));
